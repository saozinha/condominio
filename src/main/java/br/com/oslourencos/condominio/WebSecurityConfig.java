...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/").permitAll() // todos podem acessar a page de login
			/*DETERMINA QUE PARA REALIZAR ESSA REQUEST PRECISA TER UMA DAS PERMISSÕES ABAIXO
		 	* EXEMPLO DA URL: http://localhost:8095/usuario/novo 
			...
			.antMatchers("/usuario/novo").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_CADASTROUSUARIO')")
			/*DETERMINA QUE PARA REALIZAR ESSA REQUEST PRECISA TER UMA DAS PERMISSÕES ABAIXO
			 * EXEMPLO DA URL: http://localhost:8095/usuario/pesquisar*/
			.antMatchers("/usuario/pesquisar").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_CONSULTAUSUARIO')")
			 /*DETERMINA QUE PARA ACESSAR A PÁGINA INICIAL DA APLICAÇÃO PRECISA ESTÁ AUTENTICADO*/
			.antMatchers("/home").authenticated()
			.antMatchers("/usuario/novo").authenticated()
			.antMatchers("/usuario/pesquisar").authenticated()
